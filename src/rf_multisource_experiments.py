...
def train_evaluate(X, y, groups, combo_name, n_features_before):
    ...
    assert len(overlap) == 0, f"Group leakage: {overlap}"
...
def main():
    ...
    csv_path = os.path.join(RESULTS_METRICS, "rf_multisource_group_split_results.csv")
    results_df.to_csv(csv_path, index=False)
    print(f"\nSaved: {csv_path}")
    print(results_df.to_string(index=False))

    json_path = os.path.join(RESULTS_METRICS, "rf_multisource_group_split_results.json")
    ...
    if all_results:
        ...
        cm_path = os.path.join(
            RESULTS_FIGURES, "rf_multisource_group_split_confusion_matrix.png"
        )
    ...
    prev_path = os.path.join(RESULTS_METRICS, "rf_window_group_split_results.json")
